...
const Sidebar = ({ admin, router, closeSidebar, open, openSidebar }) => {
  ...
  return (
    <>
      ...
      <nav className={[styles.Sidebar, open ? styles.Open : ''].join(' ')}>
        <div className={styles.NavMenuOpener}>
          {open ? (
            <CloseIcon color='#fff' size='2em' onClick={closeSidebar} />
          ) : (
            <MenuIcon color='var(--pri)' size='2em' onClick={openSidebar} />
          )}
        </div>
        ...
        <ul className={styles.SidebarInner}>
          {pathnames.map((pathname) => {
            ...
            return (
              <li key={pathname} className={styles.SidebarItem}>
                <Link href={href}>
                  <a
